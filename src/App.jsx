...
// --- 1. CONFIGURATION (No Sample Data) ---
const INITIAL_PROJECT = {
  title: "BILL OF QUANTITIES",
  subTitle: "Project Description / Location",
  clientLabel: "Client:",
  clientName: "",
  dateLabel: "Date:",
...
const INITIAL_COLUMNS = {
  ...
  waste: { label: "WST%", visible: false }, // Default hidden
  mat: { label: "MATERIAL", visible: true },
  lab: { label: "LABOR", visible: true },
  plant: { label: "PLANT", visible: false }, // Default hidden
...
const App = () => {
  ...
  const calculatedItems = useMemo(() => {
    return items.map(item => {
      if (item.type === 'item') {
        // Formula: (Material * WasteFactor) + Labor + Plant
  ...
  const grandTotal = calculatedItems.reduce((sum, item) => sum + (item.calculatedTotal || 0), 0);
  ...
  const addItem = (type) => {
    const newItem = {
      ...
      code: '', 
      desc: type === 'section' ? 'NEW SECTION TITLE' : '', 
      unit: type === 'item' ? 'm' : '', 
      qty: 0, waste: 0, mat: 0, lab: 0, plant: 0 
    };
  ...
  const handleExcelExport = () => {
    let csv = `Code,Description,${columns.unit.label},${columns.qty.label}`;
      ...
      {isSettingsOpen && (
        <div className="bg-white border-b border-gray-200 p-6 print:hidden animate-in slide-in-from-top duration-200 shadow-lg relative z-40">
           <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
              {/* Column Visibility */}
              <div>
                <h3 className="font-bold text-sm uppercase text-slate-500 mb-3">Visible Columns</h3>
              ...
              {/* Column Renaming */}
              <div>
                <h3 className="font-bold text-sm uppercase text-slate-500 mb-3">Rename Headers</h3>
                <div className="space-y-2">
                   {Object.keys(columns).filter(k => columns[k].visible).map(key => (
                      <div key={key} className="flex items-center gap-2">
                         <span className="text-xs text-slate-400 w-12 uppercase">{key}</span>
                         <input 
                           value={columns[key].label} 
                           onChange={(e) => updateColumn(key, 'label', e.target.value)}
          ...
          <header className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-8">
             <div className="flex-1">
               ...
               <input 
                 ...
                 className="text-lg text-slate-500 w-full font-medium bg-transparent outline-none"
                 placeholder="Click to add project description or location..."
               />
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-900">
                ...
                {columns.qty.visible && <th className="p-2 text-center font-bold w-20">{columns.qty.label}</th>}
              ...
              {items.length === 0 && (
                <tr className="print:hidden">
                  <td colSpan="12" className="p-12 text-center border-b border-dashed border-gray-300">
                    ...
                    <p className="text-sm text-gray-400">Click "Add Section Header" or "Add Line Item" below to start.</p>
                  </td>
                </tr>
              )}
              ...
              {calculatedItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50 print:hover:bg-transparent group break-inside-avoid">
                    <td className="p-2 align-top"><input value={item.code} onChange={(e) => updateItem(item.id, 'code', e.target.value)} className="bg-transparent w-full text-gray-500 outline-none"/></td>
